...
### 清理音视频相关残留（项目明确不再做音视频）

- 删除 `webrtc-demo/`（独立 Node.js WebRTC 信令 demo，与主工程零耦合）与 `.qoder/`（IDE 生成的过期 wiki，含大量 webrtc 描述）
- 好友信息页删除 `voice_chat` / `video_chat` 两个未接线按钮及对应图标（7 个 PNG）、QSS 样式、qrc 条目；`msg_chat` 按钮保留并保持居中
- 删除从未使用的视频消息枚举值：客户端 `MsgType::VIDEO_MSG`、`ChatMsgType::VIDEO`，服务端 `ChatMsgType::VIDEO`；`msg_type=2` 废弃，`FILE=3` 编号不变（chat_message 白名单本就只放行 1/3）
- 文档同步：README 移除 day44/day45 音视频外链、第三季待做移除音视频三个章节、`protocol_ids.h` 与速查表去掉「18xx 预留音视频」、`sql备份` 4 处 msg_type 注释去掉「2=视频」
- 文件传输链路不受影响：mp3/mp4 仍可作为普通文件消息发送（`guessMimeType` MIME 表保留）
- 将来如需支持音视频，从 18xx 协议段重新规划加回
