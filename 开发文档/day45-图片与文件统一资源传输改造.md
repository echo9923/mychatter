...
旧图片链路（1035 登记 → 1037/1041/1043 按 seq+MD5 上传 → 1045/1047 按 seq 下载）存在的问题：

- 服务端对 MD5 零校验，无 `.part` 临时文件与原子改名，半传文件直接暴露给下载侧；
- 1045/1047 下载无权限校验，客户端可拼 `sender_id+name` 读任意文件；
...
| ID | 改造后语义 |
|---|---|
| 1035/1036 | 创建资源消息（file_name/content_hash/mime_type/msg_type=1|3），ChatServer 校验会话归属与大小上限（图 20MB/文件 100MB，[Resource] 可配） |
| 1037/1038 | 按 message_id/offset/chunk_sha256 上传分片；响应带 server_offset 与 resource_status |
| 1041/1042 | 纯上传进度查询，返回服务端 .part 实际字节数（首传/续传统一入口） |
| 1043/1044 | **删除**（旧续传分支） |
| 1045/1046 | 下载信息查询，校验请求者必须是 sender/recv |
| 1047/1048 | 按 offset 下载分片，响应携带该片 SHA-256 |
| 1039 | 通用资源消息通知（envelope 含 resource_status/content_hash/mime_type，msg_type 取 DB 真值） |
| 1031-1034 | 头像通道保持不变（旧 seq+MD5 协议） |
...
- ChatServer `DealCreateResourceMsg`（1035）校验链：fromuid==session、会话成员（新 DAO `GetPrivateChatMembers`）、msg_type∈{1,3}、大小上限、文件名 sanitize、SHA-256 格式、MIME 格式 → 幂等 UPSERT（不写同步行，上传完成才可见）。
...
- FILE_MSG 接入 outbox（`SEND_RESOURCE`，metadata→uploading 两阶段），不再被跳过；1036 后先 1041 对齐服务端偏移再窗口续发 1037；断线/重启恢复统一走 1041（1043 分支删除）。
