#include "MsgNode.h"
RecvNode::RecvNode(int max_len, short msg_type):MsgNode(max_len),
_msg_type(msg_type){

}


SendNode::SendNode(const char* msg, int max_len, short msg_type):MsgNode(max_len + HEAD_TOTAL_LEN)
, _msg_type(msg_type){
	//先发送消息类型, 转为网络字节序
