...
// ---------- 资源消息任务（1505 上传 / 1511 下载） ----------
/// 上传一个分片（1505）：data 为 Base64，offset 为该分片在整文件中的字节偏移
...
/// 下载一个分片（1511）：从最终文件（已通过 .part 原子改名）按 offset 读取
...
public:
	...
	/// 资源分片下载（1511）：必须在 ResourceWorkerIndex 选定的 worker 上执行
