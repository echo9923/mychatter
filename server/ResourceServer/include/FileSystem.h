...
public:
	...
	/// 资源分片上传（1507）：index 必须由 ResourceWorkerIndex(message_id, FILE_WORKER_COUNT) 计算
	void PostChunkToQue(std::shared_ptr<ResourceChunkTask> msg, int index);
	/// 资源分片下载（1513）：index 必须由 ResourceWorkerIndex(message_id, DOWN_LOAD_WORKER_COUNT) 计算
	void PostChunkDownToQue(std::shared_ptr<ResourceChunkDownTask> msg, int index);
	/// 任意闭包到指定 FileWorker（1509 进度查询等，与分片写入同 worker 串行化）
