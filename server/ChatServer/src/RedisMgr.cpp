...
std::string RedisMgr::Eval(const std::string& script, const std::vector<std::string>& keys, const std::vector<std::string>& args)
{
	auto connect = _con_pool->getConnection();
	if (connect == nullptr) {
		return "";
	...
	// argv: EVAL <script> <numkeys> <key...> <arg...>
	// 以 redisCommandArgv 逐元素传递，避免 %s 格式被脚本/键/参数中的空格或特殊字符拆分
	...
	argv.push_back(numkeys.c_str());  argvlen.push_back(numkeys.size());
	...
	if (reply == nullptr) {
		//不打印键/参数/回复，避免泄露票据或令牌值
		std::cout << "Execute command [ EVAL ] failure ! " << std::endl;
		_con_pool->returnConnection(connect);
		return "";
	}
	...
	if (reply->type == REDIS_REPLY_STRING || reply->type == REDIS_REPLY_STATUS) {
		...
		//REDIS_REPLY_NIL / ERROR / ARRAY 一律视为空（fail-closed）
		result = "";
	}
...
bool RedisMgr::CompareAndExpire(const std::string& key, const std::string& token, int ttl_seconds)
{
	//仅当 GET key == token 时才 SETEX 续期；不匹配返回 0，不触碰已有值
	static const char* kScript =
		"if redis.call('get',KEYS[1])==ARGV[1] then return redis.call('setex',KEYS[1],ARGV[2],ARGV[1]) else return 0 end";
	std::string ttl_str = std::to_string(ttl_seconds);
	std::string result = Eval(kScript, { key }, { token, ttl_str });
	//匹配时 Lua 返回 SETEX 的状态回复 "OK"；不匹配返回整数 0；Eval 出错返回空串
