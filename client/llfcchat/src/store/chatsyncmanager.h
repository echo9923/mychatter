...
//登录 1102/重连成功后启动；bootstrap_complete=0 先走 bootstrap（1405 取 checkpoint
//→ 1401/1402 拿会话列表 → 写库 → MarkBootstrapComplete → 进入增量）；
//增量 1405 {after_sync_seq, limit:100}，1406 整页交 applySyncPage，
...
public:
    ...
    ~ChatSyncManager();
    //登录 1102/重连成功后启动（公有 API 只 emit 信号，TCP 线程执行）
    void start();
    //1303/1505 落库成功后兜底触发一次增量拉取
    void notifyOnlineMessage();
public slots:
    //TcpMgr 转发的 1406 增量同步回包（原始 JSON 对象）
    void slot_sync_message_rsp(QJsonObject rsp);
    //TcpMgr 转发的 1402 会话列表回包（bootstrap 用）
...
private:
    ...
    enum SyncState {
        ...
        SYNC_BOOTSTRAP_THREADS, //1401/1402 拉取会话列表中
    ...
    bool _has_more_pending;     //1406 声明 has_more，等本页提交成功后续拉
