...
//头像通道（1031-1034）保持旧 seq+MD5 协议；资源消息通道（1037/1041/1045/1047）
...
public:
    ...
    //窗口发送下一批资源分片（1037）：从 MsgInfo 当前确认偏移续读。
    ...
    //启动资源下载：1045 查元数据 -> 1047 逐片下载（自动/手动触发统一入口）
...
signals:
    void sig_close();
     ...
     //资源上传收全（1038 resource_status=1），OutboxDispatcher 据此 confirmResourceSent
     void sig_resource_upload_done(QString unique_name);
     //资源上传永久失败（1026/1027 等），OutboxDispatcher 据此 markSendFailed
     ...
     //1041/1042 上传进度查询回包（OutboxDispatcher 重启恢复续传用）
