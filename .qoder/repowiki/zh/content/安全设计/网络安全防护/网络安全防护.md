...
### TCP 连接安全与会话管理
- 粘包与头部校验：读取固定长度的头部后，校验 msg_type 与 msg_len 是否越界，防止畸形报文导致内存溢出或拒绝服务。
...
HeadOK --> |是| Validate["校验msg_type/msg_len"]
Validate --> Valid{"合法?"}
Valid --> |否| CloseErr
...
- 异常行为检测：
  ...
  - 对非法 msg_type/msg_len、频繁断线重连等行为进行标记。
- 安全监控：
...
- 定位方法：
  - 查看服务端日志（read failed、length not match、invalid msg_type/msg_len）。
