...
- 现有实现未启用 TLS/HTTPS 双向认证，也未对 TCP 明文通道进行应用层签名与完整性校验。
- 本文在尊重现有实现的基础上，给出可落地的 HTTPS/TLS、双向认证、消息签名与完整性校验、防重放与会话劫持防护方案，并提供端口与防火墙建议。
...
- GateServer 提供 HTTP 入口并转发至 StatusServer/VerifyService 等 gRPC 服务。
- ChatServer 基于 Boost.Asio/Beast 处理 TCP 会话，包含心跳与超时清理。
- WebRTC 示例展示了 wss/TLS 的使用方式。

```mermaid
graph TB
Client["客户端(QT)<br/>HTTP + TCP"] --> Gate["GateServer<br/>HTTP入口"]
Gate --> Status["StatusServer<br/>gRPC"]
...
- GateServer：HTTP 接入点，调用 gRPC 获取聊天服务器地址与令牌，完成登录流程。
- ChatServer：基于 Asio/Beast 的 TCP 会话管理与心跳检测。
- 状态/验证码服务：gRPC 接口，生成/校验 Token，派发验证码。
...
## 架构总览
下图展示当前控制面与消息面的交互路径，以及未来引入 TLS/HTTPS 后的演进方向。
...
G->>S : GetChatServer(uid) -> 返回host/port/token
G->>V : 可选：验证码/邮箱校验
G-->>C : 返回token/目标ChatServer
C->>CH : 建立TCP连接(明文)
C->>CH : 携带token登录握手
CH-->>C : 登录成功，进入长连接
```
...
- 缺失项：当前未启用 TLS/HTTPS、未对 TCP 消息签名、未实现双向认证与完整性校验。
...
## 结论
当前 LLFCChat 的控制面与消息面均为明文传输，存在被窃听、篡改与重放的风险。建议在保留现有协议与心跳机制的前提下，优先引入以下措施：
- 将 HTTP 升级为 HTTPS，并为内部 gRPC 启用 mTLS。
...
### 四、TCP 连接加密与消息签名
- 方案 A：在 TCP 之上封装 TLS（如 OpenSSL/Boost.Asio SSL），透明加密。
- 方案 B：应用层加密（AES-GCM）+ HMAC-SHA256 签名，每包附带 nonce。
