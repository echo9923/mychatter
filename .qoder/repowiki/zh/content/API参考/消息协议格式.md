...
- 消息节点 MsgNode/RecvNode/SendNode：封装收发缓冲与消息ID。
- 全局常量与枚举：ReqId、MSG_IDS、ErrorCodes、ChatMsgType 等。
...
## 架构总览
TCP 帧格式（客户端与服务器一致）：
- 头部：2 字节消息ID + 2 字节载荷长度（网络字节序，大端）
- 载荷：JSON 字符串（UTF-8），包含业务字段与 error 字段
- 校验：服务端对 msg_id/msg_len 进行合法性检查；载荷通过 JSON 解析与 error 字段判断成功与否

```mermaid
flowchart TD
Start(["开始接收"]) --> CheckHead["缓冲区是否≥4字节?"]
CheckHead --> |否| WaitMore["等待更多数据"]
CheckHead --> |是| ParseHead["解析头部: msg_id(2B), msg_len(2B)"]
ParseHead --> Validate{"msg_id/msg_len 合法?"}
...
CheckErr --> |是| Dispatch["根据 msg_id 路由到处理器"]
...
- 全局枚举 ReqId 与 MSG_IDS 保持两端一致，保证路由正确

```mermaid
graph LR
Global["global.h(ReqId/枚举)"] --> Client["tcpmgr.cpp(客户端)"]
Const["const.h(MSG_IDS/常量)"] --> Server["CSession.cpp(服务器)"]
...
- 服务器：查看 CSession 的日志输出（msg_id、msg_len、read length not match）；监控 _send_que 与 _recv_msg_node
