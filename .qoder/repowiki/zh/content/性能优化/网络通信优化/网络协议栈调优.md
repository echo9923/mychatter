...
### 客户端TcpMgr（聊天消息）
- 接收流程：readyRead累积数据，循环解析固定长度的头部（消息类型+长度），再按长度读取体部，最后分发至处理器映射表
...
### 服务端CSession（会话与心跳）
- 异步读：先读固定长度头部，校验msg_type/msg_len合法性，再读体部，完成后投递到逻辑队列
