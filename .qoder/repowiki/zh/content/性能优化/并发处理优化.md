...
- LogicSystem：单工作线程消费消息队列，按 msg_id 分派到具体处理器；使用条件变量减少忙等。
...
- 设计要点
  ...
  - RegisterCallBacks() 注册 msg_id 到处理函数的映射，便于扩展。
...
- 调试建议
  - 打印关键路径日志（如 msg_id、uid、session_id、错误码）。
  - 监控队列长度、连接池大小、锁获取成功率。
  - 使用性能分析工具定位热点函数与锁竞争点。
