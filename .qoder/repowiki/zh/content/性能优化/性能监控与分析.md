...
## 引言
本技术文档围绕 LLFCChat 的性能监控与分析展开，聚焦于性能指标收集、监控数据采集与日志分析策略；阐述如何识别性能瓶颈、监控 CPU 与内存使用率；记录网络性能监控、数据库性能分析与 I/O 性能分析方法；并给出生产环境监控部署与告警机制配置建议。同时结合代码中的心跳检测、连接数统计、异步 I/O 与消息队列等实现，提供可落地的优化与观测方案。

## 项目结构
LLFCChat 采用多服务架构（聊天、网关、资源、状态、验证），其中 ChatServer 负责 TCP 长连接、会话管理与业务逻辑调度，配合 Redis 做分布式锁与连接计数，MySQL 做持久化存储。关键性能相关点包括：
...
## 架构总览
下图展示从客户端请求到服务端处理的完整链路，以及心跳检测与连接数统计的时序。
...
L->>DB : 持久化聊天记录/好友关系
SS-->>C : 返回响应
Note over S,SS : 定时器每60s扫描会话，统计活跃连接数并写回Redis
```
...
- 定时器周期：每60秒遍历会话集合，统计有效连接数并写入 Redis
...
Expired --> |否| Count["活跃计数+1"]
Count --> NextScan["继续遍历"]
Close --> NextScan
NextScan --> DoneScan["遍历结束"]
DoneScan --> WriteCount["将活跃连接数写入Redis"]
...
- 所有服务通过 Redis 共享状态（连接计数、分布式锁、用户在线映射）
...
- 指标定义与采集
  - 连接数：CServer 定时器每60秒统计活跃会话数并写入 Redis（LOGIN_COUNT）
...
- 生产环境监控部署与告警
  - 指标上报：Prometheus 抓取自定义指标（连接数、延迟、队列长度）
  - 日志采集：Fluentd/Filebeat 收集结构化日志，集中存储至 Elasticsearch
  - 可视化：Grafana 面板展示 QPS、延迟分布、错误率、资源使用
  - 告警规则：连接数突增/突降、心跳超时率>阈值、队列积压>阈值、Redis/MySQL 错误率>阈值
