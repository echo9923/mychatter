...
## 架构总览
FileTcpMgr基于Qt事件循环与QTcpSocket实现异步I/O，采用“请求ID+长度”的自定义协议进行粘包/半包处理；使用QQueue作为发送队列，bytesWritten回调驱动分段写入；通过_cwnd_size实现滑动窗口式并发控制；以MsgInfo记录每个文件的序列号、已确认序列、飞行中序列集合等，支撑断点续传与重传。
