...
## 更新摘要
**所做更改**   
- 新增了ReadDeliveryInt配置工具函数的详细说明，支持OfflinePullBatch、PullMaxBytes和OfflineTtlSeconds等键值的安全解析和回退默认值
- 增强了Delivery系统配置管理的功能描述，包括离线消息拉取、批量大小限制和过期时间设置
- 更新了配置文件结构规范，增加了[Delivery]节的相关配置项说明
- 完善了配置读取验证机制的文档，突出安全解析和异常处理逻辑
...
## 简介
本文件为 LLFCChat 项目的配置管理器（ConfigMgr）提供系统化文档，覆盖设计架构、INI 解析实现、键值存储与管理、读取与默认值处理、多环境配置支持、更新通知机制、错误处理与日志记录，以及在各服务中的集成用法与最佳实践。该配置管理器基于 Boost.PropertyTree 解析 INI 配置文件，采用单例模式暴露全局访问点，并在各服务端模块中广泛使用。**最新更新**：ConfigMgr类已添加详细的Doxygen注释，涵盖INI文件解析和基于分段的配置获取功能，提供了完整的API文档和使用说明。新增的ReadDeliveryInt函数为Delivery系统提供了专门的配置读取工具，支持离线消息拉取、批量大小限制和过期时间等关键参数的安全解析。
...
### Delivery系统专用配置工具函数
**新增功能**：ReadDeliveryInt函数为Delivery系统提供了专门的整数配置读取工具，支持安全解析和回退默认值机制。

- **函数特性**：
  - 从[Delivery]节读取指定键的整数值
  - 支持非法值、缺失值的自动回退到默认值
  - 严格的输入验证，确保值为正整数
  - 异常安全，捕获所有可能的解析异常
...
```mermaid
flowchart TD
A["ReadDeliveryInt(key, fallback)"] --> B["从ConfigMgr读取'Delivery'节的key值"]
B --> C{"值是否为空?"}
C --> |是| D["返回fallback默认值"]
C --> |否| E["尝试stoi解析为整数"]
E --> F{"解析成功且pos==val.size()?"}
F --> |否| D
F --> |是| G{"n > 0?"}
G --> |否| D
G --> |是| H["返回解析的整数值"]
...
**图表来源**
- [ChatServer/LogicSystem.cpp:40-56](file://server/ChatServer/src/LogicSystem.cpp#L40-L56)

**章节来源**
- [ChatServer/LogicSystem.cpp:40-56](file://server/ChatServer/src/LogicSystem.cpp#L40-L56)
- [ChatServer/LogicSystem.cpp:1173-1179](file://server/ChatServer/src/LogicSystem.cpp#L1173-L1179)
...
- **ChatServer**：
  ...
  - 使用ReadDeliveryInt函数读取Delivery系统配置，确保离线消息处理的灵活性。
- **GateServer**：
...
## 结论
ConfigMgr 以简洁清晰的架构实现了 INI 配置的解析与内存化管理，满足 LLFCChat 各服务的配置需求。**最新更新**：通过添加详细的Doxygen注释和新增的ReadDeliveryInt函数，显著提升了代码的可读性和可维护性，为开发者提供了完整的API文档和使用指导。新增的Delivery系统配置工具函数为离线消息处理提供了灵活的配置管理能力，支持批量大小、字节限制和过期时间等关键参数的安全解析。当前版本具备稳定的读取能力与基础的错误处理，但尚未实现环境变量支持、热重载与默认值机制。建议后续增强这些能力以提升灵活性与可运维性。
...
### Delivery系统配置详解
**新增功能**：Delivery系统配置专门用于控制离线消息的处理行为，通过ReadDeliveryInt函数提供安全的配置读取。

- **配置项说明**：
  - `OfflineTtlSeconds`：离线消息在Redis中的过期时间，单位秒，默认604800（7天）
  - `OfflinePullBatch`：单次拉取的离线消息数量上限，默认100条
  - `PullMaxBytes`：单次拉取响应的最大字节数，默认30000字节
  - `RpcDeadlineMs`：RPC请求超时时间，单位毫秒
  - `RpcMaxAttempts`：RPC重试最大次数
  - `RpcBackoffMs`：RPC重试间隔时间，单位毫秒
...
**章节来源**
- [ChatServer/LogicSystem.cpp:1173-1179](file://server/ChatServer/src/LogicSystem.cpp#L1173-L1179)
- [ChatServer/chatserver1.ini:33-36](file://server/ChatServer/config/chatserver1.ini#L33-L36)
- [ResourceServer/config.ini:27-30](file://server/ResourceServer/config/config.ini#L27-L30)