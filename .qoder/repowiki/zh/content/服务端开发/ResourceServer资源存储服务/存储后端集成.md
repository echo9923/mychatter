...
## 核心组件
- FileSystem：单例，维护多实例 FileWorker 与 DownloadWorker，按消息 ID 哈希分发任务，实现水平扩展与负载均衡。
...
## 架构总览
资源服务器通过 FileSystem 将不同业务类型的文件任务路由到对应的 FileWorker/DownloadWorker，处理器根据消息 ID 调用具体逻辑：解码 Base64、写入本地磁盘、更新数据库状态、通过 gRPC 通知聊天服务器推送消息。Redis 用于会话、令牌、下载进度等热点数据的缓存与分布式锁。
...
- 处理器注册：通过 _handlers 映射 MSG_IDS 到具体 lambda 函数。
...
- 常量定义分布在两个 const.h，区分资源服务器与聊天服务器的消息 ID、错误码与前缀。
...
### 存储后端插件架构与扩展机制
- 插件入口：FileWorker 的 RegisterHandlers 通过 _handlers 映射 MSG_IDS 到处理器。
- 扩展步骤：
  1) 新增消息 ID（在 const.h 中定义）
...
- 步骤：
  1) 在 const.h 中新增对应 MSG_IDS（如 ID_UPLOAD_OSS_REQ）
  2) 在 FileWorker::RegisterHandlers 中注册新处理器
