...
- FileWorker：上传任务处理器，基于消息类型注册处理器，支持普通文件、头像、聊天图片及续传场景；包含目录创建、分块写入、权限检查、回调通知、数据库与缓存联动。
- DownloadWorker：下载任务处理器，支持断点续传，基于 Redis 维护序列号与进度，按固定分块大小读取并 Base64 编码返回。
- FileInfo：下载断点续传的元信息模型（名称、总大小、已传输大小、序列号、路径）。
- const.h：错误码、消息类型、常量（最大分块大小、Worker 数量等）。
...
- 职责：接收 FileTask，依据 MSG_TYPES 路由到具体处理器，完成分块写入、权限校验、目录创建、数据库与缓存联动、回调通知。
- 处理器覆盖：
...
```mermaid
flowchart TD
Start(["进入task_callback"]) --> Lookup["根据msg_type查找处理器"]
Lookup --> Found{"找到处理器?"}
Found --> |否| Return["直接返回"]
...
### 使用示例（调用抽象接口）
- 上传文件：
  - 构造 FileTask（包含 session、msg_type、uid、path、name、seq、total_size、trans_size、last、file_data、callback 等）。
