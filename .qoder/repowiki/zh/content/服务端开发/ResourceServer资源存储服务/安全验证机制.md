...
## 项目结构
资源服务器侧负责文件写入与读取的并发工作线程；客户端侧负责TCP协议封装、分片发送与接收、进度与重传逻辑。常量定义集中管理错误码、消息类型、最大分片长度等。
...
- const.h：集中定义错误码、消息类型、常量（如MAX_FILE_LEN）
- data.h：用户信息与聊天数据结构
...
### 常量与错误码（const.h）
- **作用**：统一错误码、消息类型、分片大小、Redis键前缀、锁超时等
