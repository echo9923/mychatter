...
- FileWorker 负责上传类任务（普通文件、头像、聊天图片、信息同步、续传图片等），内部通过消息类型路由到具体处理器。
...
- FileWorker
  ...
  - 通过 _handlers 映射 MSG_TYPES 到具体处理函数，支持多种上传场景（文件、头像、聊天图片、信息同步、续传图片）。
...
## 架构总览
资源服务端以“多 Worker + 任务队列”为核心，结合 Redis 做状态持久化，形成高并发、可恢复的文件处理能力。客户端通过 TCP 协议与服务端交互，遵循统一的头部（type+长度）与 Base64 载荷格式，配合 seq/last/trans_size 字段实现可靠分片与断点续传。
...
- 处理器注册与路由
  - RegisterHandlers 注册多个 MSG_TYPES 对应的 lambda 处理器，如 ID_UPLOAD_FILE_REQ、ID_UPLOAD_HEAD_ICON_REQ、ID_IMG_CHAT_UPLOAD_REQ、ID_FILE_INFO_SYNC_REQ、ID_IMG_CHAT_CONTINUE_UPLOAD_REQ。
  - task_callback 根据 _msg_type 查找处理器并调用。
- 通用上传流程
...
```mermaid
flowchart TD
Start(["进入task_callback"]) --> Lookup["根据_msg_type查找处理器"]
Lookup --> Found{"找到处理器?"}
...
- 粘包解析
  - 读取头部（消息类型+长度），再按长度读取消息体，循环处理直到缓冲区不足。
- 发送队列与拥塞控制
