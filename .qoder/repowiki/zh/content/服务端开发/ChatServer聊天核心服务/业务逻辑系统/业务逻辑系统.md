...
- 消息路由表：按 msg_id 将请求分发至对应处理器（登录、搜索、好友申请/认证、文本/图片聊天、心跳、加载线程/消息等）。
...
扩展点
- 新增消息类型：在 RegisterCallBacks 中注册 msg_id 到处理器函数；在 DealMsg 中自动路由。
...
FindCB --> |是| Call["调用处理器(session,msg_id,data)"]
Call --> Done["出队并继续"]
Wait --> EnqCheck
Next --> EnqCheck
```
...
- 识别：依据 msg_id 路由到具体处理器（登录、搜索、好友、文本/图片、心跳、线程/消息加载）。
...
### 扩展点设计
- 新增消息类型
  - 在 const.h 的 MSG_IDS 枚举中添加新 msg_id。
