...
- 常量与协议：const.h 定义错误码、消息 ID、Redis Key 前缀等
...
- LogicSystem：消息分发中心，注册各 MSG_ID 的处理回调，实现登录、好友、聊天等逻辑
- MysqlMgr/MysqlDao：数据库访问抽象与实现，提供用户、好友、聊天消息等 CRUD
- RedisMgr：Redis 连接池、基础命令封装、分布式锁、计数器等
- const.h：统一错误码、消息 ID、Redis Key 前缀、锁超时等常量
