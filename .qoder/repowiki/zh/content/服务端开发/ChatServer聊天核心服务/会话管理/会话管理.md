...
- MsgNode/RecvNode/SendNode：消息头体封装，统一头部长度与ID，支持网络字节序转换与缓冲复用。
- LogicSystem：单例逻辑调度器，内部维护消息队列与工作线程，按消息ID分发给具体处理器。
...
- 结构设计
  ...
  - RecvNode：继承 MsgNode，增加 _msg_id，用于接收消息。
  - SendNode：继承 MsgNode，在构造时将 msg_id 和长度转为网络字节序并拼接头部，便于直接发送。
- 缓冲区管理
...
ReadHead --> ParseHead["解析头部(msg_id, msg_len)"]
...
- 读回调
  ...
  - AsyncReadHead 解析头部，校验 msg_id/msg_len，分配 RecvNode 并继续读取 Body。
- 写回调
...
## 附录
- 协议常量与消息ID定义见 const.h，数据结构定义见 data.h。
