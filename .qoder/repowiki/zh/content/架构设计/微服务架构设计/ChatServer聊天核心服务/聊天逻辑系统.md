...
- [CSession.h:1-212](file://server/ChatServer/include/CSession.h#L1-L212)
- [LogicSystem.h:1-187](file://server/ChatServer/include/LogicSystem.h#L1-L187)
...
## 核心组件
- **LogicSystem**：单例，维护消息队列与回调映射；启动独立工作线程消费队列；按msg_id分发给具体处理器（登录、搜索、好友申请/认证、文本/图片消息、心跳、线程加载等）。采用生产者-消费者模式，IO线程负责消息接收和入队，工作线程负责业务处理和出队。
...
### 生产者-消费者模式实现
LogicSystem实现了经典的生产者-消费者模式：
- **生产者**：IO线程（CSession）负责接收客户端消息，封装为LogicNode后通过PostMsgToQue入队
- **消费者**：独立的工作线程（_worker_thread）负责从队列中取出消息并处理
- **同步机制**：使用互斥锁（_mutex）保护队列访问，条件变量（_consume）实现线程间通信
- **优雅关闭**：通过_b_stop标志位确保服务关闭时能正确处理剩余消息

**更新** 新增详细的Doxygen注释说明生产者-消费者模式的实现细节

章节来源
- [LogicSystem.h:171-185](file://server/ChatServer/include/LogicSystem.h#L171-L185)
...
章节来源
- [LogicSystem.cpp:254-275](file://server/ChatServer/src/LogicSystem.cpp#L254-L275)
- [LogicSystem.cpp:577-585](file://server/ChatServer/src/LogicSystem.cpp#L577-L585)
...
**更新** LogicSystem类的Doxygen注释完善后，代码的可读性和可维护性显著提升，为后续的功能扩展和维护提供了清晰的指导。
