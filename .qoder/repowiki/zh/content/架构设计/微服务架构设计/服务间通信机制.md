...
## 更新摘要
**所做更改**
- 移除了不安全的明文传输，所有gRPC通信现在都通过mTLS加密
- StatusServer的Login方法已标记为遗留死路径，始终返回TokenInvalid错误
- 更新了安全认证章节以反映新的mTLS实现
- 修正了架构图以显示mTLS加密连接
- 增强了故障排查指南中的安全相关问题
...
**更新** 当前架构已实现完整的mTLS加密通信和可配置的跨服务交付重试机制，通过统一的Delivery配置管理超时控制、最大重试次数和指数退避策略，显著提升了系统的可靠性和安全性。
...
章节来源
- [ChatServer.cpp:1-115](file://server/ChatServer/src/ChatServer.cpp#L1-L115)
...
- gRPC 协议与服务
  ...
  - StatusService：获取 ChatServer 路由信息、登录校验（已废弃）。
  - VarifyService：发送验证码。
- **更新** 可配置的gRPC客户端架构
  - ChatGrpcClient：使用共享Channel映射管理多个目标节点的gRPC通道，支持可配置的重试机制和指数退避。
  - ChatServerGrpcClient（ResourceServer）：集成类似的可靠通知机制，支持图片消息的可靠投递。
  - StatusGrpcClient（ChatServer/GateServer）：使用单个共享Channel连接到StatusServer，现已启用mTLS加密。
  - VerifyGrpcClient：使用单个共享Channel连接到VarifyServer。
- 服务端实现
  - ChatServiceImpl：实现 ChatService 接口，转发到内部逻辑系统。
  - StatusServiceImpl：维护 ChatServer 注册表与 Token 映射，提供路由与登录（Login方法已废弃）。
- 配置与常量
  - const.h：错误码、消息 ID、分布式锁相关常量。
  - config.ini：各服务监听端口、依赖服务地址、Delivery重试配置。

**更新** 所有gRPC客户端现在都支持可配置的Delivery重试机制，通过统一的配置项管理超时、重试次数和退避策略，确保跨服务调用的可靠性。StatusServer的GetChatServer方法已启用mTLS验证。
...
- StatusServer 维护 ChatServer 注册信息与 Token，为客户端提供路由与鉴权（Login方法已废弃）。
...
Verify-->>Gate : 验证码结果
Gate->>Status : gRPC GetChatServer (mTLS加密, 3s超时)
Status-->>Gate : ChatServer路由信息
Gate-->>Client : 返回结果
...
Note over Gate,Status : mTLS双向认证
```
...
### StatusService 协议与消息
- 服务方法
  - GetChatServer：根据 uid 获取 ChatServer 地址与 token（已启用mTLS验证）
  - Login：登录校验（已废弃，始终返回TokenInvalid错误）
- 消息体
...
Note : Login方法已废弃<br/>始终返回TokenInvalid错误
```
...
- **StatusGrpcClient（ChatServer/GateServer）**
  ...
  - **更新** 已启用mTLS加密：通过环境变量加载CA证书、客户端证书和私钥
  - 暴露 GetChatServer、Login 方法（Login已废弃）
  - 所有调用都包含3秒超时控制
- **VerifyGrpcClient**
...
StatusGrpcClient --> Channel : "mTLS加密通道"
VerifyGrpcClient --> Channel : "单一共享通道"
```

**更新** 架构实现了完整的可配置重试机制和mTLS加密通信，通过统一的Delivery配置管理所有gRPC调用的超时、重试和退避策略，确保跨服务调用的可靠性和安全性。
...
- StatusServiceImpl
  ...
  - **更新** 已启用mTLS验证：验证客户端证书的SAN字段必须包含"llfc-gate"
  - **更新** Login方法已废弃：始终返回TokenInvalid错误
- 启动流程
...
- 常量
  ...
  - MSG_TYPES：自定义 TCP 消息类型（用于内部消息路由）。
  - 分布式锁前缀与超时参数。
...
- **更新** mTLS加密仅应用于GateServer与StatusServer之间的通信。
- 配置集中管理，便于多实例部署与动态切换。

```mermaid
graph LR
Gate["GateServer"] --> Status["StatusServer<br/>mTLS加密"]
Gate --> Verify["VarifyServer<br/>明文"]
...
Note1 -.-> Res
Note2["mTLS环境配置<br/>LLFC_STATUS_CA_CERT_PATH<br/>LLFC_GATE_CLIENT_CERT_PATH<br/>LLFC_GATE_CLIENT_KEY_PATH"] -.-> Gate
```
...
**更新** 新架构通过可配置的Delivery重试机制和mTLS加密，显著提升了系统的可靠性和安全性，确保了跨服务调用的稳定性和机密性。

[本节为通用指导，不直接分析具体文件]

## 安全与认证
- **更新** 传输加密
  - **已启用mTLS**：GateServer与StatusServer之间的通信现在使用双向TLS认证
  - GateServer通过环境变量加载CA证书、客户端证书和私钥
  - StatusServer验证客户端证书的SAN字段必须包含"llfc-gate"
  - 其他服务间通信仍使用明文传输（生产环境应全面启用TLS）
- **更新** 身份认证
  - StatusServer 的 Login 方法已废弃，始终返回TokenInvalid错误
  - 推荐使用GetChatServer方法进行路由获取，该方法已启用mTLS验证
  - 后续请求可使用ticket机制进行鉴权
- 权限控制
...
**更新** 安全架构已升级为mTLS双向认证，有效防止中间人攻击和数据窃听。Login方法的废弃消除了潜在的安全风险。
...
- **更新** 常见问题
  ...
  - **新增** mTLS相关问题：检查环境变量是否正确设置，证书文件是否存在且可读，SAN字段是否匹配。
  - **新增** 重试相关问题：检查Delivery配置是否正确，重试次数是否合理，退避时间是否合适。
  - 超时问题：所有gRPC调用都有可配置的超时限制，超时会导致RPCFailed错误码。
  - 认证失败：Token 无效或过期，检查 StatusServer 登录流程与存储（注意Login方法已废弃）。
...
## 结论
LLFCChat 的微服务通信以 gRPC 为核心，通过**可配置的跨服务交付重试机制**和**mTLS双向认证**实现了高效、可靠且安全的服务间调用。**更新后的架构**具有以下优势：

- **可靠性提升**：实现了完整的Delivery重试机制，支持可配置的超时控制、最大重试次数和指数退避策略
- **安全性增强**：GateServer与StatusServer之间已启用mTLS双向认证，有效防止中间人攻击和数据窃听
...
当前实现已具备基础的跨服务可靠通信能力和增强的安全保障，但在其他服务间通信的TLS加密、熔断降级、负载均衡与可观测性方面仍有改进空间。建议在生产环境中逐步补齐这些能力，以提升系统的稳定性、安全性和可维护性。

**更新** 新的可配置重试机制和mTLS加密为系统的可靠性和安全性提供了坚实保障，能够有效应对网络波动、服务临时不可用等异常情况，确保跨服务调用的最终一致性和数据传输机密性。