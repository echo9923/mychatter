...
- 业务逻辑系统（LogicSystem）
  ...
  - 关键点：按msg_id分派处理器、批量消息处理、线程安全队列。
- Redis管理器（RedisMgr）
...
### 客户端TCP管理器（TcpMgr）
- 粘包/拆包
  - 读取时先解析固定长度的头部（消息ID+长度），再按长度读取消息体，避免粘包。
- 发送队列
...
CheckHead -- 否 --> ReadHead["读取头部(消息ID+长度)"]
...
### 业务逻辑系统（LogicSystem）
- 消息路由
  - RegisterCallBacks注册各msg_id对应的处理器，如DealChatTextMsg、DealChatImgMsg、HeartBeatHandler等。
- 工作队列
...
- 消息ID
  - const.h中MSG_IDS定义了客户端-服务端消息ID映射，用于路由与分发。
