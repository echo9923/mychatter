...
## 简介
本文件面向 LLFCChat 的消息路由与分发系统，重点说明基于 ReqId 的客户端侧请求-响应路由机制，以及服务器端基于 msg_id 的消息分发流程。内容涵盖：
- 客户端 ReqId 映射表、处理器注册、异步回调处理
- 服务器端 msg_id 到业务处理器的注册与调用
...
S_LogicSystem["LogicSystem<br/>msg_id->回调注册与分发"]
end
C_TcpMgr --> |TCP: ID+Len+Body| S_CSession
C_FileTcpMgr --> |TCP: ID+Len+Body| S_CSession
...
- 服务端 MsgNode/RecvNode/SendNode：定义协议帧头（ID、长度）与缓冲管理，统一编解码格式。
- 服务端 LogicSystem：维护 msg_id 到业务回调的映射表，单线程消费队列并调用对应处理器，实现解耦与顺序执行。
...
## 架构总览
下图展示从客户端发送数据到服务器业务处理的端到端流程，包括 ReqId/msg_id 的映射、协议帧封装、I/O 读写、队列投递与回调分发。
...
- 发送流程：构造 ReqId 与数据块，写入网络字节序的 ID 与长度，进入发送队列并发出信号。
...
### 服务器端 msg_id 路由机制（CSession + LogicSystem）
- 协议帧：头部包含 msg_id（短整型，网络字节序）与数据长度（短整型或长整型，依服务而定）。
- 读流程：CSession 先读 HEAD_TOTAL_LEN，校验合法性后创建 RecvNode，再异步读取 Body。
- 投递：将 (session, recvnode) 封装为 LogicNode，投递至 LogicSystem 队列。
- 分发：LogicSystem 单线程消费队列，按 msg_id 查找回调并调用业务处理器。
...
- 服务端接收：CSession 解析头部，校验长度与 ID，构建 RecvNode。
- 队列投递：CSession 将 LogicNode 投递给 LogicSystem。
- 业务处理：LogicSystem 按 msg_id 查找回调并执行，必要时调用外部服务（如 Redis/Mysql/gRPC）。
...
- 服务端：若 msg_id 非法或长度超限，关闭会话并清理资源；若未找到回调，记录日志并丢弃。
...
- 服务器回调注册：在 RegisterCallBacks 中将 msg_id 绑定到业务处理器。
- 调试建议：
  - 客户端：打印 handleMsg 的 ReqId 与数据，确认映射是否正确。
  - 服务端：打印 DealMsg 中的 msg_id 与回调查找结果，定位未注册或解析错误。
  - 参考开发文档中的示例代码片段路径。
...
- 常见问题：
  ...
  - 服务端未找到回调：检查 RegisterCallBacks 是否绑定 msg_id。
  - 粘包/半包：确认头部长度与体部读取一致。
...
## 结论
LLFCChat 的消息路由与分发系统通过客户端 ReqId 与服务端 msg_id 的双映射机制，实现了清晰、可扩展的请求-响应路由。CSession 与 LogicSystem 的职责分离确保了高内聚低耦合，结合队列与异步 I/O 提升了吞吐与稳定性。未来可在此基础上扩展优先级、重试与熔断等高级特性。

## 附录
- 协议常量：MSG_IDS 定义了所有消息类型 ID，便于统一管理与扩展。
