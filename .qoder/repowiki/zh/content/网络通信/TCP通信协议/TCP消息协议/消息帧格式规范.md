...
- 协议常量（消息 ID、头部长度等）在服务端 const.h 中统一定义；业务消息体采用 JSON 序列化，部分服务间通信使用 gRPC（chat.proto）。
...
- 服务端：
  ...
  - const.h：HEAD_TOTAL_LEN、HEAD_ID_LEN、HEAD_DATA_LEN、MAX_LENGTH、MSG_IDS 等协议常量。
...
## 架构总览
LLFCChat 的 TCP 帧采用“固定长度头部 + 变长消息体”的结构，头部包含消息 ID 和消息体长度，消息体为 JSON 字符串。客户端与服务端在各自侧完成字节序转换与粘包/拆包处理。
...
Client->>Net : "写入帧 : [ID : 2B][Len : 2B][Body : JSON]"
Note over Client,Server : "头部为大端网络字节序"
Net-->>Server : "到达数据流"
Server->>Server : "读取HEAD_TOTAL_LEN=4字节"
Server->>Server : "解析ID与Len(网络->本地字节序)"
...
### 字节序与大小端转换
- 客户端发送：
  - 使用 QDataStream 设置 BigEndian 后写入 ID 与长度
- 服务端接收：
...
- 客户端（FileTcpMgr）：
  - 头部 6 字节，解析 ID(2B)+Len(4B)，同样按长度边界拆分
- 服务端（CSession）：
...
### 帧结构示例（文本消息）
- 头部：
  - ID: 2 字节（例如 ID_TEXT_CHAT_MSG_REQ）
  - Len: 2 字节（Body 长度）
- 消息体（JSON 片段）：
...
### 帧结构示例（图片通知）
- 头部：
  - ID: 2 字节（ID_NOTIFY_IMG_CHAT_MSG_REQ）
  - Len: 2 字节（Body 长度）
- 消息体（JSON 片段）：
  - thread_id、sender_id、receiver_id、message_id、img_name、total_size
...
### 帧结构示例（文件传输）
- 头部：
  - ID: 2 字节（例如 ID_IMG_CHAT_DOWN_REQ）
  - Len: 4 字节（Body 长度）
- 消息体：
