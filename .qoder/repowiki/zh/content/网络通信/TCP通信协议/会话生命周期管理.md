...
- 服务端 LogicSystem：消息队列+工作线程，按MSG_ID路由到具体处理器（登录、搜索、好友、聊天、心跳、加载聊天等）。
...
- 接收与粘包处理：
  - 使用缓冲区累积数据，先读固定长度头部（消息ID+长度），再按长度读取体。
...
- 异步读写：
  - AsyncReadHead读取固定长度头部，校验msg_id/msg_len合法性后分配RecvNode并进入AsyncReadBody。
...
- 客户端ReqId：
  - global.h中与服务器MSG_IDS保持一致，用于客户端侧消息分发。
- gRPC接口：
