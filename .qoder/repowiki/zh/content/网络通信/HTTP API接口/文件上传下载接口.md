...
说明：当前仓库中 GateServer 的 LogicSystem 未包含文件上传/下载的 HTTP 路由注册；实际文件上传/下载由 ResourceServer 的 FileWorker 通过内部消息类型驱动。本文档基于现有代码进行归纳与扩展设计，确保与既有实现一致并可指导客户端集成。
...
## 核心组件
- FileWorker：上传/下载任务队列与工作线程，按消息类型分发到具体处理器，完成 Base64 编解码、文件 IO、Redis 状态维护、MySQL 状态更新、gRPC 通知。
...
- 字段说明：
  - msg_type：消息类型（ID_UPLOAD_FILE_REQ、ID_UPLOAD_HEAD_ICON_REQ、ID_IMG_CHAT_UPLOAD_REQ、ID_FILE_INFO_SYNC_REQ、ID_IMG_CHAT_CONTINUE_UPLOAD_REQ）
  - uid：用户ID
  - seq：分块序号（从1开始递增）
  - total_size：文件总大小（字节）
...
- 排查步骤：
  - 检查请求字段是否完整（msg_type、uid、seq、last、path、name、file_data）。
  - 确认目录权限与磁盘空间。
