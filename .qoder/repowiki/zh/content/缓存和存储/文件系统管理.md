...
- const.h：定义错误码、消息 ID、常量（如 MAX_FILE_LEN、工作线程数量等）。
...
- 协议与粘包处理：自定义头部（ID+长度），循环解析缓冲区，确保完整消息体后再分发到处理器。
...
```mermaid
flowchart TD
Start(["开始"]) --> ParseHead["解析消息头(ID+长度)"]
...
- 调试步骤
  - 查看日志：服务端打印文件路径、分块大小、错误码；客户端打印消息 ID、长度、进度。
