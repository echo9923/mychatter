...
**更新** RedisConPool类现在包含详细的Doxygen注释，说明了连接池的生命周期管理和线程安全机制。
...
**更新** DistLock类现在包含完整的Doxygen注释，详细说明了分布式锁的工作原理和使用方法。
...
### 命令封装与错误处理
- **所有命令**从连接池获取上下文，执行后统一 freeReplyObject 并归还连接。
...
**更新** 登录计数相关的四个方法（IncreaseCount、DecreaseCount、InitCount、DelCount）现在都有完整的Doxygen注释，说明了负载均衡统计的实现原理。
...
**更新** 连接池的健康检查机制现在有更详细的注释说明，包括失败连接的重试策略和资源清理流程。
