...
- 协议定义：聊天相关 RPC 接口在 chat.proto 中定义；TCP 帧头采用固定长度（ID+Length），JSON 作为消息体。
...
UI --> TcpMgr
TcpMgr --> |TCP 帧: ID+Length+JSON| CSession
...
- 服务端 LogicSystem：单例工作线程消费消息队列，按 msg_id 路由到具体处理器，完成登录、好友、聊天、图片等逻辑，并与 Redis/Mysql 交互。
- 协议与常量：全局 ReqId/MSG_IDS 枚举、错误码、头部长度、最大队列长度等。
...
Client->>Server : "连接建立"
Client->>Server : "登录请求(ID=LOGIN, JSON)"
...
Client->>Server : "文本聊天请求(ID=TEXT_REQ, JSON)"
Server->>Logic : "投递聊天消息"
Logic->>DB : "落库/缓存"
...
- 粘包/半包解析：readyRead 中将所有数据追加到缓冲区，循环解析头部（消息ID+长度），再读取消息体，确保边界正确。
...
- 异步读：AsyncReadHead 读取固定长度头部，校验 msg_id/msg_len，分配 RecvNode 并进入 AsyncReadBody 读取消息体。
...
- 工作线程：DealMsg 从 _msg_que 取出 LogicNode，按 _recvnode->_msg_id 查找回调并执行。
- 回调注册：RegisterCallBacks 将 MSG_IDS 映射到具体处理方法（登录、搜索、好友申请、认证、聊天、心跳、加载线程/消息、图片消息）。
...
### 协议与消息确认机制
- 帧格式：头部固定长度（ID+Length），消息体为 JSON。客户端与服务端均按此格式解析与组装。
- 消息ID：客户端使用 ReqId 枚举，服务端使用 MSG_IDS 枚举，两者保持一致。
