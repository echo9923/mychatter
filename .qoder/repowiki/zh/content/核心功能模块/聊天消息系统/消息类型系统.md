...
## 更新摘要
**所做更改**
- 新增 MsgNode 类的 GetMsgId() 方法章节，详细说明 IO 线程消息路由机制
...
**最新更新**：增强了服务器端消息处理能力，通过 MsgNode 类的 GetMsgId() 方法暴露消息类型ID给IO线程，支持ReqToRspId()函数进行请求响应消息ID映射，提升了系统的稳定性和错误处理能力。
...
S_MsgNode["MsgNode.h/cpp<br/>GetMsgId()方法"]
...
- **新增**：MsgNode 类族：包括基类 MsgNode、接收节点 RecvNode 和发送节点 SendNode，提供统一的内存管理和消息ID访问接口。
...
Note over Server : "新增：MsgNode.GetMsgId()暴露消息ID"
...
- **RecvNode 接收节点**：存储从客户端接收的完整消息，包含消息ID和数据体，**新增 GetMsgId() 方法**暴露消息类型ID给IO线程。
- **SendNode 发送节点**：存储待发送给客户端的消息，包含消息头（ID+长度）和数据体。

**关键特性**：
- GetMsgId() 方法允许 IO 线程获取消息类型ID，用于路由分片绑定（计划1.3）
- 支持网络字节序转换，确保跨平台兼容性
- 智能内存管理，避免内存泄漏
...
- 服务端侧
  ...
  - **新增**：使用 MsgNode 类族管理消息节点，支持 GetMsgId() 获取消息类型ID。
- 客户端侧
...
ParseHeader --> CreateNode["创建 RecvNode 并设置 msg_id"]
CreateNode --> GetMsgId["GetMsgId() 获取消息类型ID"]
GetMsgId --> Dispatch{"根据 ReqId 分发"}
...
- 路由机制
  ...
  - **新增**：服务器端使用 MsgNode.GetMsgId() 进行 IO 线程的路由分片绑定。
...
Render --> REnd(["完成"])
Note over RStart,REnd : "服务器端：MsgNode.GetMsgId() 用于路由分片"
```
...
- 服务端层
  ...
  - **新增**：const.h 中的 MSG_IDS 枚举和 ReqToRspId() 映射函数
...
- 添加步骤
  ...
  - **新增**：在 const.h 的 MSG_IDS 枚举中添加新的消息ID，并在 ReqToRspId() 中添加映射关系。
- 兼容性处理
...
- **新增**：服务器端依赖
  - MsgNode 类族依赖 const.h 中的消息ID定义和网络字节序转换。
...
- **新增**：服务器端性能优化
  ...
  - MsgNode.GetMsgId() 支持 IO 线程的路由分片，提升并发处理能力。
...
**新增特性总结**：
- MsgNode.GetMsgId() 方法为 IO 线程提供了消息类型ID访问能力
