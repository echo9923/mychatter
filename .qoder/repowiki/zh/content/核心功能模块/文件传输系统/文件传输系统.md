...
## 简介
本文件为 LLFCChat 文件传输系统的功能与技术文档，覆盖客户端与服务端在文件上传、下载、断点续传、进度显示等方面的实现细节。重点说明调用关系、协议字段、领域模型与使用模式，并结合实际代码路径给出示例引用。系统采用 TCP 分块传输（Base64 编码）、服务端工作线程池与 Redis 持久化状态，支持大文件分块、并发控制、断点续传与暂停/恢复。
...
- FileWorker / DownloadWorker（服务端）
  - 基于线程池的任务队列，分别处理上传与下载任务；下载侧通过 Redis 维护断点信息，按 seq 偏移读取并 Base64 返回。
- FileSystem（服务端）
...
- 服务端由 FileSystem 选择具体 Worker，FileWorker 解码 Base64 写入磁盘；DownloadWorker 根据 seq 和 Redis 中的断点信息定位偏移量读取数据并返回。
- 下载断点状态保存在 Redis，确保进程重启或网络中断后可恢复。
...
FT->>RS : ID_IMG_CHAT_DOWN_REQ(JSON)
RS->>FS : PostDownloadTaskToQue(task,index)
FS->>FW : PostTask(task)
FW->>RD : 读取/更新下载断点信息
FW-->>FT : ID_IMG_CHAT_DOWN_RSP(JSON, data, seq, is_last)
FT-->>UM : 更新进度/完成回调
UM-->>UI : 刷新进度/完成
```
...
- 职责
  ...
  - 上传流程：组织分块 JSON（name、seq、trans_size、total_size、last、data、last_seq、uid 等），等待服务器回包后继续下一块。
  - 下载流程：发起下载请求，接收 Base64 数据并写入本地文件，按 seq 追加，最后通知完成。
  - 断点续传：根据 last_confirmed_seq 与 rsp_seqs 集合计算已确认序列，必要时重发未确认块。
...
- 职责
  ...
  - 与 UI 交互，更新进度条与完成状态。
- 关键数据结构
  - MsgInfo：包含 total_size、current_size、seq、md5、rsp_seqs、flighting_seqs、last_confirmed_seq、max_seq、transfer_state、transfer_type、sender/receiver 等。
...
**章节来源**
- [global.h:152-200](file://client/llfcchat/include/global.h#L152-L200)
- [usermgr.cpp:375-528](file://client/llfcchat/src/usermgr.cpp#L375-L528)

### 服务端：FileWorker / DownloadWorker
- 职责
  - FileWorker：处理上传类请求（头像、聊天图片、通用文件），Base64 解码后按 seq 写入磁盘，最后一个包时更新数据库状态并通过 gRPC 通知 ChatServer。
  - DownloadWorker：按 seq 从指定偏移读取文件，Base64 编码后返回；首次下载初始化 FileInfo 并写入 Redis，后续续传从 Redis 读取断点信息校验 seq 与偏移。
- 关键点
...
```mermaid
flowchart TD
Start(["收到下载任务"]) --> CheckSeq{"seq == 1 ?"}
CheckSeq --> |是| InitInfo["读取文件大小<br/>构造 FileInfo<br/>写入 Redis"]
CheckSeq --> |否| LoadInfo["从 Redis 读取断点信息"]
LoadInfo --> ValidateSeq{"seq 匹配?"}
ValidateSeq --> |否| ErrSeq["返回序列号无效错误"]
ValidateSeq --> |是| CalcOffset["计算偏移 offset = (seq-1)*MAX_FILE_LEN"]
InitInfo --> CalcOffset
...
CleanRedis --> Return["返回 data/seq/is_last/total_size/current_size"]
UpdateRedis --> Return
ErrSeq --> Return
```
...
- 关键点
  - 构造函数中初始化固定数量的工作线程实例。
  - PostMsgToQue / PostDownloadTaskToQue 将任务投递到对应 worker。
...
- 服务端
  - FileWorker/DownloadWorker 依赖 const.h（MSG_IDS、ErrorCodes、MAX_FILE_LEN）、RedisMgr（断点状态）、MysqlMgr（更新状态）、ChatServerGrpcClient（通知）。
...
- 断点续传
  - 下载侧以 Redis 存储 FileInfo（seq、trans_size、total_size、file_path_str），保证跨进程/重启恢复。
- 分块大小
...
- 常见问题
  ...
  - 下载进度不更新：确认 ID_IMG_CHAT_DOWN_RSP 处理逻辑是否被调用，检查 seq 与 is_last 字段。
  - 断点续传失败：检查 Redis 中是否存在断点信息，确认 seq 是否一致。
...
- [FileWorker.cpp:537-660](file://server/ResourceServer/src/FileWorker.cpp#L537-L660)
- [const.h:5-29](file://server/ResourceServer/include/const.h#L5-L29)

## 结论
LLFCChat 的文件传输系统通过客户端 FileTcpMgr 与服务端 FileWorker/DownloadWorker 的协作，实现了稳定可靠的大文件分块传输、断点续传与进度反馈。借助 Redis 的状态持久化与分布式锁，系统在异常恢复与并发控制方面具备良好鲁棒性。建议在后续迭代中完善超时重传机制与更细粒度的拥塞控制策略，进一步提升稳定性与吞吐。
...
### 协议字段（上传/下载）
- 上传请求（ID_IMG_CHAT_UPLOAD_REQ / ID_IMG_CHAT_CONTINUE_UPLOAD_REQ）
  - name：文件名
  - seq：分块序号
  - trans_size：当前累计传输字节数
  - total_size：文件总大小
  - last：是否为最后一块
  - data：Base64 编码的分块数据
  - last_seq：上次发送的序号（用于断点）
  - uid：当前用户 ID
- 上传响应（ID_IMG_CHAT_UPLOAD_RSP / ID_IMG_CHAT_CONTINUE_UPLOAD_RSP）
  - error：错误码
  - name、seq、receiver、sender：上下文信息
- 下载请求（ID_IMG_CHAT_DOWN_REQ）
  - name、seq、trans_size、total_size、token、sender_id、receiver_id、message_id、uid
- 下载响应（ID_IMG_CHAT_DOWN_RSP）
  - data：Base64 编码的分块数据
  - seq、is_last、total_size、current_size、name、req_type（头像下载时携带）

**章节来源**
- [filetcpmgr.cpp:243-899](file://client/llfcchat/src/filetcpmgr.cpp#L243-L899)
- [FileWorker.cpp:196-453](file://server/ResourceServer/src/FileWorker.cpp#L196-L453)

### 领域模型
- MsgInfo（客户端）
  - 字段：msg_type、text_or_url、unique_name、total_size、current_size、seq、md5、rsp_seqs、flighting_seqs、last_confirmed_seq、max_seq、msg_id、rsp_size、thread_id、transfer_state、transfer_type、sender、receiver
- DownloadInfo（客户端）
