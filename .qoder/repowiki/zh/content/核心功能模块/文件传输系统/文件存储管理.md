...
- const.h：错误码、消息ID、常量（如 MAX_FILE_LEN、工作者数量等）。
...
- FileWorker 内部维护消息处理器映射表，根据消息ID路由到相应处理逻辑（上传、头像、聊天图片、续传等）。
...
### FileWorker 与消息处理器
- 设计模式：注册式处理器映射（_handlers），以 MSG_IDS 为键，函数对象为值。
- 主要处理器：
...
```mermaid
flowchart TD
Start(["进入 task_callback"]) --> Lookup["根据 _msg_id 查找处理器"]
Lookup --> Found{"找到处理器?"}
Found -- "否" --> End["直接返回"]
